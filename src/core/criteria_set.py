...
class CriteriaSet(object):
    ...
    def edit_criteria(self, crit_id, criteria_info):
    	...
    	self.table.update(self.table.c.criteria_id==crit_id).execute({'description':desc, 'type':type, 'type_options':type_options, 'cost_benefit':cost_benefit})
    	print crit_id
    	print str(criteria_info)
    ...
    def get_criteria_by_description(self, desc):
        result = self.table.select(self.table.c.description==desc).execute()
        if result:
            criteria_row = result.fetchone()
            crit_data = list(criteria_row)
            crit_data[3] = pickle.loads(criteria_row[3])
            return crit_data        
